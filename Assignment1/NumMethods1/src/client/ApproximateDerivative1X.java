...
public class ApproximateDerivative1X 
{
	// Function f(x) = 1/x
	public static BigDecimal function(BigDecimal x)
	{
	    return BigDecimal.ONE.divide(x, MathContext.DECIMAL128);
	}
	
	// Derivative of 1/x = -1/x^2
	public static BigDecimal realDerivative(BigDecimal x)
	{
		return BigDecimal.ONE.divide(x.pow(2), MathContext.DECIMAL128).negate();
	}
	...
	public static void createGraph(BigDecimal x, int n)
	{
		...
		for (int i = 1; i <= n; i++)
		{
			...
			BigDecimal approxDerivative = approximateDerivativeEquation(ApproximateDerivative1X::function, x, h);
		...
		JFreeChart lineGraph = ChartFactory.createXYLineChart("Derivative Approximation Error for f(x)=1/x", "-log_10(h)", "log_10(|Error|)", dataset);
		...
		JFrame windowGraph = new JFrame("Derivative Approximation Errors 1/x");
	...
	{
		// Approximate the derivative of f(x) = 1/x at x = 0.5
